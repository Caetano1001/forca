...
buttons[i][3])
                    pygame.draw.circle(win, buttons[i][0], (buttons[i][1],
buttons[i][2]), buttons[i][3] - 2
                                       )
...
buttons[i][2] - (label.get_height() / 2)))
        ...
        def buttonHit(x, y):
            for i in range(len(buttons)):
                    if (x < buttons[i][1] + 20 and x > buttons[i][1] - 20 and 
                        y < buttons[i][2] + 20 and y > buttons[i][2] - 20):
                        return buttons[i][5]
            return None
       ...
       # screen.fill((0,0,0))
        ...
        #draw_text('Meu Jogo', font, (155, 255, 155), screen, 20, 20)
      #Aqui mudei a cor do texto da outra parte
        #for event in pygame.event.get():
           # if event.type == QUIT:
             #   pygame.quit()
           #     sys.exit()
